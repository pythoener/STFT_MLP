...
def plot_spectrogram(signal, sample_rate, title='Spectrogram'):
    D = librosa.amplitude_to_db(np.abs(librosa.stft(signal, n_fft=670, hop_length=335)), ref=np.max)
...
def UpsampledAudio(audio, fs):
    audio_resampled = librosa.resample(audio, orig_sr=fs, target_sr=8000) #After this step i want that we save the audio
...
target_folder = r'E:\Raw Data\timit_bwe\train\clean'
...
def reconstruct_audio(lowaudio, sample_rate, highAudio, mirrored_low_audio):
    ...
    X_windows_norm = feature_scaler.transform(low_stft_db.T)
    ...
    y_pred_log_mag = label_scaler.inverse_transform(pred_windows)
    ...
    cutoff_bin = int(4000 / (sample_rate / 670))
    ...
    reconstructed_signal = librosa.istft(stft_complex_combined, hop_length=335, win_length=670, window='hamming')
...
def processfile(file_path):
    ...
    try:
        ...
        save_audio(reconstructed_audio, file_path, r'C:\Users\HPC3\PycharmProjects\mert\STFT_MLP/Results_TIMIT_SHIFTED', fs)
...
test_folder_low = r'E:\Raw Data\TIMIT_BWE_SF\04_test_clean\shifted'
